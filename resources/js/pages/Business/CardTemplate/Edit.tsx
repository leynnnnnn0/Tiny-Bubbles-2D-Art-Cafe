...
export default function Edit({ cardTemplate }) {
  const { data, setData, put, processing, errors } = useForm({
    ...
    stampImage: cardTemplate.stampImage ? `/${cardTemplate.stampImage}` : null,
    backgroundImage: cardTemplate.backgroundImage ? `/${cardTemplate.backgroundImage}` : null,
          ...
          <div className="lg:col-span-3 space-y-4 md:space-y-6">
            ...
            <div className="hidden md:block">
              <Button type="submit" className="w-full" size="lg" disabled={processing}>
                ...
                {processing ? 'Updating Template...' : 'Update Card Template'}
              </Button>
            </div>
          </div>
                    ...
                    <div className="p-4 sm:p-6 md:p-8 backdrop-blur-sm" style={{ backgroundColor: data.backgroundImage ? 'rgba(0,0,0,0.2)' : 'transparent' }}>
                      ...
                      <div className="grid grid-cols-5 gap-2 md:gap-3 mb-4 md:mb-6">
                        {Array.from({ length: data.stampsNeeded }).map((_, index) => {
                          ...
                          const isFilled = index < Math.floor(data.stampsNeeded * 0.7);
